...
var event3_type = "A";
var step2_answer = ['개구리', '코끼리', '악어'];
var step3_answer = [1,2,2];
if(Math.floor((Math.random() * 2) + 1) == 2){
    event3_type = "B";
    step2_answer = ['산', '다리', '학교'];
    step3_answer = [3,1,3];
}
...
if(location.protocol.indexOf('https') == -1){
	...
	$('.mobile_submit_btn_wrap').css('display', 'block');   //  .mobile_submit_btn_wrap style.css display:none; -> block 시키기
});
...
function quiz_step3(){
    ...
    if(event3_type == "A"){//step3_answer 123
        ...
        $("#step3_"+event3_type+" input").on("click", function(idx) {
            $('.check').fadeOut('fast'); // 모든체크이미지 OFF
            $(this).next().children().fadeIn('fast');// 클릭된 체크 이미지 ON
            now_val_arr[now_step] = $(this).val();

        });         
    }

    $('#step3_'+event3_type+' .submit_wrap').click(function() {
        if(event3_type == "B"){
            if(now_val_arr[now_step] == step3_answer[now_step]){
                if(now_step==2){
                    popup.open('#step3_answer_pop_'+event3_type);
                }else{
                    $("#step3_"+event3_type+" #step3_" + (now_step + 1)).hide();
                    $("#step3_"+event3_type+" #step3_" + (now_step + 2)).show();  
                    $('#step3_'+event3_type+' .submit_wrap a img').attr('src', 'img/submit_btn_off.png');
                    now_step++;                     
                }
            }else if(now_val_arr[now_step] == 0){
                alert('정답을 선택해주세요');
            }else{
                $('#step3_'+event3_type+' .submit_wrap a img').attr('src', 'img/submit_btn_on.png');
                popup.open('#incorrect_pop');   
            }
                    
        }else{
             if(step3_q1 == 0 || step3_q2 == 0 || step3_q3 == 0 ){
                alert('정답을 모두 선택해주세요.');
            }else{
                if(step3_q1 == step3_answer[0] && step3_q2 == step3_answer[1] && step3_q3 == step3_answer[2] ){
                     popup.open('#step3_answer_pop_'+event3_type);
                }else{
                    popup.open('#incorrect_pop');
                    $('#step3_'+event3_type+' .submit_wrap a img').attr('src', 'img/submit_btn_on.png');
...
function quiz_step3_mobile(){
    ...
    $('#mobile_step3_'+event3_type+'_wrap .mobile_submit_btn_wrap').click(function() {
        if(event3_type == "B"){
            if(now_val_arr[now_step] == step3_answer[now_step]){
                if(now_step==2){
                    popup.open('#mobile_sub3_answer_step3_'+event3_type);
                }else{
                    $("#mobile_step03_"+event3_type+" #mobile_step3_" + (now_step + 1)).hide();
                    $("#mobile_step03_"+event3_type+" #mobile_step3_" + (now_step + 2)).show();  
                    $('#mobile_step3_'+event3_type+'_wrap .mobile_submit_btn_wrap img').attr('src', 'img/mobile_submit_btn_off.png');
                    now_step++;                     
                }
            }else if(now_val_arr[now_step] == 0){
                alert('정답을 선택해주세요');
            }else{
                $('#mobile_step3_'+event3_type+'_wrap .mobile_submit_btn_wrap img').attr('src', 'img/mobile_submit_btn_on.png');
                popup.open('#mobile_incorrect_pop');   
            }
                    
        }else{
             if(step3_q1_m == 0 || step3_q2_m == 0 || step3_q3_m == 0 ){
                alert('정답을 모두 선택해주세요.');
            }else{
                if(step3_q1_m == step3_answer[0] && step3_q2_m == step3_answer[1] && step3_q3_m == step3_answer[2] ){
                     popup.open('#mobile_sub3_answer_step3_'+event3_type);
                }else{
                    popup.open('#mobile_incorrect_pop');
                    $('#mobile_step3_'+event3_type+'_wrap .mobile_submit_btn_wrap img').attr('src', 'img/mobile_submit_btn_on.png');
